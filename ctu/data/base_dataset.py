...
Modified from:
  https://github.com/NVlabs/SPADE/blob/master/data/base_dataset.py
"""
...
def get_transform(opt, params, method=Image.BICUBIC, normalize_mean=None, normalize_std=None, normalize=True, to_tensor=True):
  ...
  if 'resize' in opt.preprocess_mode:
    ...
    transform_list.append(transforms.Lambda(lambda img: _scale_width(img, opt.load_size, method)))
  elif 'scale_shortside' in opt.preprocess_mode:
    transform_list.append(transforms.Lambda(lambda img: _scale_shortside(img, opt.load_size, method)))

  if 'crop' in opt.preprocess_mode:
    transform_list.append(transforms.Lambda(lambda img: _crop(img, params['crop_pos'], opt.crop_size)))
  ...
  if normalize:
    ...
    transform_list += [transforms.Normalize(normalize_mean,
                                            normalize_std)]
