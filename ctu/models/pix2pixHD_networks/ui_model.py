...
class UIModel(BaseModel):
  ...
  def change_labels(self, click_src, click_tgt): 
    ...
    self.fake_image = misc.tensor2im(self.single_forward(self.net_input, self.feat_map))
      ...
      if idx.size():          
        if style_id == -1:
          ...
          for cluster_idx in range(self.opt.multiple_output):
            ...
            fake_image = misc.tensor2im(fake_image[:,min_y:max_y,min_x:max_x])
          ...
          if instToChange > 1000:
            ...
            for cluster_idx in range(self.opt.multiple_output):  
              ...
              fake_image = misc.tensor2im(fake_image[:,min_y-y_start:max_y-y_start,min_x-x_start:max_x-x_start])
              self.fake_image.append(fake_image)
          else:
            ### downsample
            ...
            for i in range(self.opt.multiple_output):
              self.fake_image.append(misc.tensor2im(fake_image_batch[i,:,min_y:max_y,min_x:max_x]))"""
  ...
  def get_current_visuals(self, getLabel=False):                
    ...
    if getLabel: # only output label map if needed to save bandwidth
      label = misc.tensor2label(self.net_input.data[0], self.opt.label_nc)          
